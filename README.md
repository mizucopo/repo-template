...
生成されるrelease workflowはRerunnable releaseです。汎用releaseとChrome Extension releaseはmainへのpushごとにcommit SHAで独立したrunを保持します。Docker releaseは先行する未完了runをGitHub Actions APIの全ページから確認し、指数backoff付きでrun番号順に待機するため、待機runを落とさず共有 `latest` tagの更新を直列化します。初回runはキュー順に `latest` を更新し、過去runの再実行は新しいrelease runが実行中なら拒否し、すでに新しいrelease runが存在する場合はversioned imageだけを補完します。手動実行もmain以外のrefでは停止します。各runは永続化済みの状態を確認して不足工程だけを再開します。GitHub Releaseの照会はHTTP 200だけを存在、404だけを未作成として扱います。同名tagが別commitを指す場合、GitHub Releaseだけが存在する場合、API・認証・通信に失敗した場合は、既存状態を未作成とみなさず安全側に失敗します。
